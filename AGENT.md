...
| `stylesheet.css` | All St CSS.  Active tile/chip color fallback is `#E95420`; overridden at runtime by a dynamically loaded CSS file from `/tmp`. |
...
- **Do NOT use `actor.style` (inline styles) for `background-color`** — in GNOME
  Shell's St toolkit, CSS class rules beat inline styles (opposite of browsers).
- Active tile/chip color is driven by a **dynamically written CSS file**:
  `_applyAccentCss(color)` in `panel.js` writes `/tmp/gnome-switch-accent.css`
  (two rules: `.gs-tile.active` and `.gs-chip.active`) and loads it via
  `St.ThemeContext.get_for_stage(global.stage).get_theme().load_stylesheet(file)`.
  Because it loads after `stylesheet.css`, its rules win at equal specificity.
  `_clearAccentCss()` unloads the file on extension destroy.
- `YARU_ACCENT` map + `_accentColorFor(iface)` resolve the current Yaru variant
  (from `org.gnome.desktop.interface gtk-theme`) to a hex color.
  `org.gnome.desktop.interface accent-color` does NOT exist on Ubuntu 24.04 / GNOME 46.
- `SwitchTile` and `PowerModeRow` only add/remove the `active` CSS class — no inline styles.
- `stylesheet.css` keeps `background-color: #E95420` on `.gs-tile.active` /
  `.gs-chip.active` as a static fallback for Yaru-dark (the default theme).
...
- Active tile hover color is not themed (dynamic CSS file could add a `:hover` rule
  with a lightened accent variant if desired).
